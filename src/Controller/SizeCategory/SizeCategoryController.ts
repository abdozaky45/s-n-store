...
import { CreateSizeCategory, deleteSizeCategory, findAllSizeCategories, findSizeCategoriesByName, findSizeCategoryById, updateSizeCategory } from "../../Service/SizeCategory/SizeCategoryService";
...
export const getSizeCategoriesByName = asyncHandler(async (req: Request, res: Response) => {
    const { name } = req.params as { name: string };
    const sizeCategories = await findSizeCategoriesByName(name);
    return res.status(200).json(new ApiResponse(200, { sizeCategories }, SuccessMessage.SIZE_CATEGORY_FETCHED));
});
